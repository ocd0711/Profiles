...
async function all() {
  if (html == undefined || !html.includes('</html>')) {
    ...
    lk.log('初始化工具栏完成')
    if (ck == "") {
      lk.msg('', '请订阅 boxjs 之后进行相关配置')
      lk.log('请订阅 boxjs 之后进行相关配置')
      lk.done({ body: html })
    } else {
      // 请求接口获取京粉转链之后的 url
      lk.log('准备开始请求jf转链')
      ...
      let options = {
        url: `https://api.m.jd.com/api?functionId=ConvertSuperLink&appid=u&_=${Date.now()}&body=${encodeURI(JSON.stringify(jfbody))}&loginType=2`,
        headers: {
          Cookie: ck,
