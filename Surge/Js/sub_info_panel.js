...
(async () => {
  let info = await getDataInfo($persistentStore.read("airport_url"));
  if (!info) $done();
  let resetDayLeft = getRmainingDays(parseInt($persistentStore.read("airport_rest_day")));

  let used = info.download + info.upload;
  let total = info.total;
  let expire = $persistentStore.read("airport_expire") || info.expire;
  ...
  $done({
    title: `${sub_title} | ${hour}:${minutes}`,
    content: content.join("\n"),
    icon: $persistentStore.read("airport_icon") || "airplane.circle",
    "icon-color": $persistentStore.read("airport_color") || "#007aff",
  });
})();

function getUserInfo(url) {
  let method =  $persistentStore.read("airport_method") || "head";
