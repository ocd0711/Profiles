/**************************************
项目名称：霸王茶姬签到
使用说明：手动签到一次，若提示获取ck即可使用，建议开启抓包再签到。
项目作者：@ocd0711

感谢樱花佬的脚本框架@sliverkiss https://t.me/sliverkiss

@Date:2024-09-09 完成脚本
...
霸王茶姬获取token = type=http-request,pattern=^https:\/\/webapi2\.qmai\.cn\/web\/cmk-center\/sign\/takePartInSign,requires-body=0,max-size=0,script-path=https://raw.githubusercontent.com/ocd0711/Profiles/master/Surge/Js/bwcj.js
----------------------------------------------
**************
*Quantumult X*
**************
[task_local]
30 9 * * * https://raw.githubusercontent.com/ocd0711/Profiles/master/Surge/Js/bwcj.js, tag=霸王茶姬签到, img-url=https://raw.githubusercontent.com/ocd0711/Profiles/master/icon/bwcj.png, enabled=true

[rewrite_local]
# 签到
^https:\/\/webapi2\.qmai\.cn\/web\/cmk-center\/sign\/takePartInSign url script-request-body https://raw.githubusercontent.com/ocd0711/Profiles/master/Surge/Js/bwcj.js
#获取ck
^https:\/\/webapi2\.qmai\.cn\/web\/cmk-center\/sign\/takePartInSign url script-request-header https://raw.githubusercontent.com/ocd0711/Profiles/master/Surge/Js/bwcj.js
        ...
        try {
            const options = {
                ...
                headers: {
                    ...
                    "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 18_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.51(0x18003326) NetType/4G Language/en",
                    "Referer": "https://servicewechat.com/wxafec6f8422cb357b/188/page-frame.html"
                },
                body: JSON.stringify({
            ...
            if (result?.code !== 0 || result?.message !== "ok" || result?.status !== true) {
                const message = `❌签到异常: ${result?.message}`;
                $.log(message);
                $.signMsg = message;
                $.returnCode = -1;
                ...
                if (result.message === "ok")
                    message = `✅签到成功: ${result?.userId || '签到完成'}`;
                else
                    message = `✅签到成功: ${result?.message}`;
                $.log(message);
                $.signMsg = message;
            }
        } catch (e) {
...
async function getCookie() {
    if ($request && $request.method === 'POST') {
        ...
        const storeId = jsonBody.storeId
        const activityId = jsonBody.activityId;
        const signature = jsonBody.signature;
        const appid = jsonBody.appid;
        const timestamp = jsonBody.timestamp;

        // 检查所有参数是否都存在
        if (token && storeId && activityId && signature && appid, timestamp) {
            // 存储参数到对象中
            const tokenData = {
                token,
                storeId,
